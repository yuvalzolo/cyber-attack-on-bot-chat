...
function updateFile() {
    setTimeout(() => {
        const chatLog = document.querySelector("#botChat").value;
        const data = { chatLog: chatLog };
        fetch('update.php', {
            method: 'POST',
...
function myFunction() {
    const searchTerm = document.querySelector("#fname").value;
    ...
    fetch('search.php', {
        ...
        .then(data => {
            console.log(data);
            const modalText = document.querySelector("#lblResponse");
			modalText.innerHTML="";
            data.forEach(sentence => {
                modalText.innerHTML += `<i>${sentence}</i></br>`
            });
			
        });
	
    $('#botChat').highlightWithinTextarea({
        highlight: searchTerm
    });
    
	$('#botChat').highlightWithinTextarea('update');
	
}

var you = "Me";
botSays("Please enter your question");
...
var Passwords = ["what's your password account", "PASSWORD"]
var arr = []
...
function answer(x) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        else if (question.includes(Passwords[i])) { Else = false; setTimeout(botSays, 600, "\nBot : The password is qwe\n") }
	...
	localStorage.setItem("chatkey", document.getElementById("botChat").value);
}

function botSays(chatLog) {
        document.getElementsByTagName("textarea")[0].innerHTML += chatLog;
}
